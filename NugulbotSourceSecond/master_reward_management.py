    ...
    def read_delete_reward(self):
        ...
        if delete_button:
            ...
            if len(selected_row) == 0:
                ...
                if delete_flag:
                    st.session_state['prior_message'] = self.message_loader.get_property("common.message.success").format("보상 삭제를")
                    st.session_state['prior_status'] = "success"
                    st.rerun()
                else:
                    self.message_loader.get_property("common.message.error").format("보상 삭제를")



    def create_update_reward(self):
        st.session_state['current_page'] = "create_update_reward"
        comu_id = st.session_state['comu_id']
        server_id = st.session_state['server_id']
        
        basic_path = "RewardManagement.create_update_reward."
        common_basic_path = "common."
        api_path = "reward"
        ...
        if send_dict:
            ...
            if send_dict.get('_id', None):
                ...
                if response:
                    ...
                    UtilRenderer.show_message(self.message_loader.get_property("common.message.error").format("보상 추가를"), "error")
