...
class MasterMonsterActiveSkillManagement:
    ...
    def read_delete_monster_active_skill(self):
        st.session_state['current_page'] = "read_delete_monster_active_skill"
        comu_id = st.session_state['comu_id']
        server_id = st.session_state['server_id']
        
        basic_path = "MasterMonsterActiveSkillManagement.read_delete_monster_active_skill."
        common_basic_path = "common."
        api_path = "monster/skill/active"
        ...
        if data_list is not None and f'{collection_name}_list' in data_list.keys():
            ...
            edited_df = data_editor_renderer.render(basic_path+"table", value_df)
        ...
        if submit_button:
            if len(selected_row) == 0:
                st.session_state['current_page'] = 'create_update_monster_active_skill'
        ...
        if delete_button:
            ...
            if len(selected_row) == 0:
                ...
                if delete_flag:
                    st.session_state['prior_message'] = self.message_loader.get_property("common.message.success").format("몬스터 스킬 삭제를")
                    st.session_state['prior_status'] = "success"
                    st.rerun()
                else:
                    self.message_loader.get_property("common.message.error").format("몬스터 스킬 삭제를")

    def create_update_monster_active_skill(self):
        st.session_state['current_page'] = "create_update_monster_active_skill"
        comu_id = st.session_state['comu_id']
        server_id = st.session_state['server_id']
        
        basic_path = "MasterMonsterActiveSkillManagement.create_update_monster_active_skill."
        common_basic_path = "common."
        api_path = "monster/skill/active"
        ...
        if send_dict:
            ...
            if send_dict.get('_id', None):
                ...
                if response:
                    st.session_state['current_page'] = 'read_delete_monster_active_skill'
                    st.session_state['selected_row'] = None
                    st.session_state['prior_message'] = self.message_loader.get_property("common.message.success").format("몬스터 스킬 업데이트를")
                    st.session_state['prior_status'] = "success"
                    st.rerun()
                else:
                    UtilRenderer.show_message(self.message_loader.get_property("common.message.error").format("몬스터 스킬 업데이트를"), "error")
            else:
                response = api.make_request(api_path, data=send_dict, method='POST')
                if response:
                    st.session_state['current_page'] = 'read_delete_monster_active_skill'
                    st.session_state['selected_row'] = None
                    st.session_state['prior_message'] = self.message_loader.get_property("common.message.success").format("몬스터 스킬 추가를")
                    st.session_state['prior_status'] = "success"
                    st.rerun()
                else:
                    UtilRenderer.show_message(self.message_loader.get_property("common.message.error").format("몬스터 스킬 추가를"), "error")